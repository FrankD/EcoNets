function [A, model, movieObj] = communitySimulation3(sim,com),
...
for i=1:sim.iter,  % Loop through all the iterations
    ...
    for n=1:prod(com.nSites),  % Loop through each resource patch
        ...
        An_Site = real(A) * model.dispersalMatrix(:,n);
        %        tmp(tmp<1e-10) = 0;
        ...
        demoStoch = sqrt(com.e_sigma)*sprandn(An_Site);

...
if exist(simData.filename.model,'file') | exist([simData.filename.model '.mat'],'file'),
    ...
    [model.webTopology, model.trophicLevel] = ...
        nicheModel(comData.nSpecies,comData.connectance,false,simData.RandomSeed.niche, simData);
...
if m<eps,
    ...
    [ind1,ind2]  = find(dM>1e-10);
    M = sparse(ind1,ind2,dM(sub2ind(size(dM),ind1,ind2)),prod(nSites),prod(nSites),length(ind1));
